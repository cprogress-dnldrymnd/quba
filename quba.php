...
class Quba_Controllers
{
    ...
    /**
     * Injects the Alternative Qualification Title into the native WordPress search SQL query block.
     * @param string $where The native SQL WHERE block sequence.
     * @param WP_Query $wp_query The executing context instance variable matrix.
     * @return string Modified WHERE clause mapping array constraints dynamically.
     */
    public static function alt_title_search_filter($where, $wp_query)
    {
        global $wpdb;
        $search_term = $wp_query->get('quba_alt_search_term');

        if ($search_term) {
            $like = '%' . $wpdb->esc_like($search_term) . '%';

            // Construct a highly-efficient EXISTS subquery mapped to the custom meta key
            $meta_subquery = $wpdb->prepare(
                " OR EXISTS (SELECT 1 FROM {$wpdb->postmeta} WHERE post_id = {$wpdb->posts}.ID AND meta_key = '_alternativequalificationtitle' AND meta_value LIKE %s)",
                $like
            );

            // Map the injection precisely into the first standard Title LIKE node generated by WP Core
            $where = preg_replace(
                "/\(\s*{$wpdb->posts}.post_title\s+LIKE\s*('[^']+')\s*\)/",
                "({$wpdb->posts}.post_title LIKE $1 $meta_subquery)",
                $where
            );
        }
    ...
    public static function archive_ajax_qualifications()
    {
        ...
        if (!empty($_POST['qualificationTitle']) && $_POST['qualificationTitle'] !== 'e') {
            $clean_search = sanitize_text_field($_POST['qualificationTitle']);
            $args['s'] = $clean_search;
            // Inject custom query var to trigger our SQL modifier
            $args['quba_alt_search_term'] = $clean_search;
        }
        ...
        add_filter('posts_where', [__CLASS__, 'alt_title_search_filter'], 10, 2);

        $query = new WP_Query($args);

        // Dismount filter immediately to prevent bleed into global scope matrices
        remove_filter('posts_where', [__CLASS__, 'alt_title_search_filter'], 10);
